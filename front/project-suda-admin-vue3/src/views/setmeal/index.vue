...
import type { SetmealRecord, SetmealQuery } from '@/api/modules/setmeal'
...
const tableData = ref<SetmealRecord[]>([])
const tableRef = ref()
const selectedIds = ref<number[]>([])

// Fetch categories (type=2 for setmeal)
...
// Selection
function handleSelectionChange(rows: SetmealRecord[]) {
...
// Edit
function handleEdit(row: SetmealRecord) {
  router.push(`/setmeal/edit/${row.id}`)
}

// Delete single
async function handleDelete(row: SetmealRecord) {
  try {
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm(`确认删除选中的 ${selectedIds.value.length} 个套餐?`, '批量删除确认', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
...
// Toggle status
async function handleToggleStatus(row: SetmealRecord, value: boolean) {
  const newStatus = value ? 1 : 0
  try {
    const res = await toggleSetmealStatus(newStatus, row.id)
    if (res.data.code === 1) {
