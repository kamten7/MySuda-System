...
<view class="ai-page">
	...
	<view class="nav" :style="{ paddingTop: sb + 'px' }">
		<view class="nav-in">
			<view class="nav-l"><view class="nav-logo">🤖</view><text class="nav-tit">AI 小速</text></view>
			<view class="nav-r">
				<view class="nav-clr" v-if="msgs.length" @click="handleClear"><text>🗑️</text></view>
			</view>
		</view>
	</view>
	...
	<scroll-view
		...
		:style="{ top: navH + 'px', height: mscH + 'px' }"
	>
		<!-- 欢迎 -->
...
export default {
	...
	methods:{
		...
		calc(){
			...
			this.mscH=h-this.navH-this.barH
		},
		send(t){
			var m=(t||'').trim()
...
/* === nav === */
.nav{position:fixed;top:0;left:0;right:0;z-index:99;background:linear-gradient(135deg,#1e3a8a 0%,#1a56db 50%,#2563eb 100%);box-shadow:0 4rpx 24rpx rgba(26,86,219,.2)}
.nav-in{height:50px;display:flex;align-items:center;justify-content:space-between;padding:0 24rpx}
.nav-l{display:flex;align-items:center;gap:12rpx}
.nav-logo{font-size:36rpx;width:56rpx;height:56rpx;border-radius:50%;background:rgba(255,255,255,.18);display:flex;align-items:center;justify-content:center}
.nav-tit{font-size:34rpx;font-weight:700;color:#fff}
.nav-r{display:flex;gap:10rpx}
.nav-clr{width:56rpx;height:56rpx;border-radius:50%;background:rgba(255,255,255,.12);display:flex;align-items:center;justify-content:center;font-size:28rpx}

/* === msg scroll === */
.msc{position:fixed;left:0;right:0;overflow-y:auto;box-sizing:border-box}

/* === welcome === */
.w{padding:48rpx 16rpx 60rpx;display:flex;flex-direction:column;align-items:center}
...
.wci{width:120rpx;height:120rpx;border-radius:50%;background:linear-gradient(135deg,$b,$bl);display:flex;align-items:center;justify-content:center;margin-bottom:28rpx;box-shadow:0 8rpx 40rpx rgba($b,.3)}
...
.wpi{display:flex;align-items:center;gap:12rpx;padding:20rpx 28rpx;background:#fff;border:1rpx solid #e5e7eb;border-radius:24rpx;box-shadow:0 2rpx 12rpx rgba(0,0,0,.03)}
...
.aa{background:linear-gradient(135deg,#dbeafe,#bfdbfe);margin-right:16rpx}
.aa .avt{font-size:26rpx;font-weight:800;color:$b}
.au{background:linear-gradient(135deg,#fbbf24,#f59e0b);margin-left:16rpx;color:#fff;font-size:24rpx}
...
.bA{background:#fff;border-top-left-radius:8rpx;box-shadow:0 2rpx 12rpx rgba(0,0,0,.04)}
...
.bb{flex:1;background:#f5f6f8;border-radius:28rpx;padding:0 28rpx}
